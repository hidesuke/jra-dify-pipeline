...
```
2026-09-13 場:06 1R ｜ 馬場[9月13日（日曜）7時00分] クッション値8.2 / 芝:重(含水率 ゴール前13.5 4角14.8) / ダート:重(含水率 ゴール前14.1 4角15.4) / 当日雨量1mm
```
...
ゴール前と 4 コーナーで区分が同じなら `芝:良(...)`、異なれば地点別に表記します。
...
これらは **Shift_JIS** なので `TextDecoder("shift_jis")` で復号します（Cloudflare Workers ランタイムでサポートを確認済み）。1 ファイルに開催中の全場が入り、会場は `title`（会場名）で識別されるため、場コードへマッピングしています。
